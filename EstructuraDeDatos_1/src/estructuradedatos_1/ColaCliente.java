...
public class ColaCliente {
    ...
    public boolean trasladoYOrganizacionDeMujeresMayorMenor1() {
    ...
    while (true) {
        ...
        while (!this.estaVacia()) {
            NodoCliente cliente = this.sacarInicio();           
            if (cliente.getDato().getSexo() == 'F') {
                hayMujeres = true;               
                if (mayor == null || cliente.getDato().getEdad() > mayor.getDato().getEdad()) {
                    if (mayor != null) {
                        temp.meterFinal(mayor);
                    }
                    mayor = cliente;
                } else {
                    temp.meterFinal(cliente);
                }
            } else {
                temp.meterFinal(cliente);
            }
        ...
        while (!temp.estaVacia()) {
            this.meterFinal(temp.sacarInicio());
        }
    }  
    return true;
}
